  ...
  return (
    <div className="productsRight">
      ...
      <div className="right__pagination">
        <button className="pagi__back__btn" onClick={backBtn}><LeftOutlined /></button>
        {
          numbers.map((number, index) => (
            <li className={`pagi__center__numbers ${data === number ? "active" : ""}`} key={index}>
              <button className="pagi__link" onClick={() => changeNumber(number)}>{number}</button>
            </li>
          ))
        }
        <button className="pagi__next__btn" onClick={nextBtn}><RightOutlined /></button>
      </div>
    </div>
  );
